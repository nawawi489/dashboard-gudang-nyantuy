...
export default function DatabaseFieldDropdown({ label, fieldName, value, onChange, placeholder }: Props) {
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      setItems(Array.from(values).sort())
    } catch (e) {
      console.error(`Gagal mengambil data ${fieldName}`, e)
      setError(`Tidak dapat memuat data ${label.toLowerCase()}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div ref={dropdownRef} className="control dropdown">
      ...
      <input
        className="input"
        placeholder={placeholder || `Cari atau pilih ${label.toLowerCase()}`}
