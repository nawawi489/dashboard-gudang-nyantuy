...
import { SPV_OUTLETS } from '../../constants'
...
const OutletSelector = ({ onStart, initialOutlet = '', initialDate = '', onLogout }: OutletSelectorProps) => {
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center p-6">
      <div className="max-w-md w-full mx-auto space-y-8">
        <div className="flex items-center justify-end">
          {onLogout && (
            <button onClick={onLogout} className="text-xs text-slate-500 hover:text-slate-800 underline" type="button">
              Logout
            </button>
          )}
        </div>
