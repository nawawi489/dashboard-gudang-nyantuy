...
import { fetchPerlengkapanSpvList, confirmPerlengkapan } from '../../services/perlengkapanSpv'
...
interface PerlengkapanDashboardProps {
  outlet: string
  onBack: () => void
}

const PerlengkapanDashboard = ({ outlet, onBack }: PerlengkapanDashboardProps) => {
  ...
  const reloadData = async () => {
    ...
    setError(null)
    try {
      const data = await fetchPerlengkapanSpvList(outlet)
      setList(data)
      setConfirmedIds([])
      setCurrentPage(1)
      setActive(null)
    } catch (e) {
      setError('Gagal memuat data perlengkapan masuk')
    } finally {
      setLoading(false)
    }
  }
  ...
  const submitConfirm = async (data: {
    ...
    try {
      ...
      await confirmPerlengkapan(payload)
      ...
      <div className="bg-white shadow-sm border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-md mx-auto px-4 h-16 flex items-center justify-between gap-3">
           ...
           <div className="flex-1">
             <h1 className="font-bold text-slate-800 text-lg">Konfirmasi Perlengkapan Masuk</h1>
             <div className="text-xs text-slate-500">{outlet}</div>
           </div>
          <button onClick={reloadData} aria-label="Reload Data" className="p-2 text-slate-600 hover:bg-slate-50 rounded-full transition-colors">
            <RefreshCw size={20} />
          </button>
        </div>
      </div>
          ...
          <div className="space-y-3">
            {list.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-slate-400 space-y-3 border-2 border-dashed border-slate-200 rounded-xl bg-white/50">
                ...
                <p>Tidak ada perlengkapan menunggu konfirmasi.</p>
              </div>
            ) : (
              <>
                <div className="space-y-3">
                  {paginatedList.map((item, idx) => (
                    <PeralatanCard
                      ...
                      confirmLabel="Konfirmasi Perlengkapan"
                    />
                  ))}
                </div>
...
export default PerlengkapanDashboard
