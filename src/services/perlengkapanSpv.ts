...
export const fetchPerlengkapanSpvList = async (outlet?: string): Promise<PeralatanItem[]> => {
  try {
    const url = new URL(SPV_ENDPOINTS.GET_PERLENGKAPAN_LIST)
    if (outlet) {
      url.searchParams.append('outlet', outlet)
    }
    const response = await fetch(url.toString())
    if (!response.ok) {
      throw new Error(`Failed to fetch perlengkapan list: ${response.status} ${response.statusText}`)
    }
    const raw = await response.json()
    return normalizePeralatanList(raw)
  } catch (error) {
    console.warn('API Error (fetchPerlengkapanSpvList):', error)
    throw error
  }
}

export const confirmPerlengkapan = async (data: PeralatanConfirmData): Promise<void> => {
  try {
    ...
    const response = await fetch(SPV_ENDPOINTS.CONFIRM_PERLENGKAPAN, { method: 'POST', body: formData })
    if (!response.ok) {
      throw new Error(`Failed to submit confirmation: ${response.status} ${response.statusText}`)
    }
  } catch (error) {
    console.warn('Error confirming perlengkapan:', error)
    throw error
  }
}
